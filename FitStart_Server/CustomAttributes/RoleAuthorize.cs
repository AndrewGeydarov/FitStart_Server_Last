...
namespace FitStart_Server.CustomAttributes
{
    ...
    public class RoleAuthorize : Attribute, IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            {
                context.Result = new JsonResult(new { error = "Токен авторизации не передан" })
                {
            ...
            string token = header.ToString().Trim();
            ...
            var session = await dbContext.Sessions.Include(x => x.User).FirstOrDefaultAsync(x => x.Token == token);

            if (session == null)
            {
                context.Result = new JsonResult(new { error = "Токен авторизации не действителен" })
                {
                    StatusCode = StatusCodes.Status401Unauthorized
                };
                return;
